...
- Worker 运行且浏览器打开时，可直接查看已穿戴装备与属性合计、浏览和对比背包装备、穿戴装备，并按稀有度和品质规则批量分解；不需要停止服务后手动登录游戏。
...
装备页从当前 Playwright 登录会话读取游戏的 gear 列表，不切换游戏页面，因此可以在自动钓鱼运行期间使用。页面以紧凑网格展示 9 个穿戴槽位和已穿戴属性合计；背包装备显示名称、稀有度、品质、强化等级、非零属性和分解价值。装备名称与边框使用游戏原版稀有度色值，品质也沿用游戏的分档颜色。

背包支持名称、槽位、稀有度筛选和分页，并直接显示当前装备相对同槽位已穿戴装备的品质和属性变化。批量分解筛选可以为每个稀有度独立启用品质上限，一键查看或选择全部命中装备；规则保存在当前浏览器中。已穿戴或锁定的装备不会命中规则，也不能加入分解列表。穿戴戒指时需要明确选择戒指 1 或戒指 2，并分别显示两个戒指槽位的对比。分解前会再次确认，服务端也会在已登录页面中重新读取装备列表，拒绝已过期、已穿戴或已锁定的选择。
