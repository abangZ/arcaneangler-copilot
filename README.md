...
- Web 控制台按概览、收益和日志分区展示日常状态，低频设置使用独立页面。
- 从游戏页面已经产生的 `/api/game/cast` 响应统计今日和累计鱼获、金币、经验、遗物、宝箱与装备收益。
...
Web 概览显示今日抛竿、鱼获、金币和经验；收益页显示今日特殊收获、累计收益、收获分类和最近 7 天数据。统计按服务器本地日期分组，保存在 `.data/stats.json`，最多保留 90 个每日汇总。
