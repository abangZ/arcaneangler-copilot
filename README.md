...
- 通过 Web 页面查看 Worker、浏览器、挂机调度、当前功能、累计抛竿和实时日志。
...
Web 登录使用一次性随机 salt/nonce、PBKDF2 和 HMAC 验证；浏览器不会把原始密码作为请求正文发送。登录成功后，服务端下发带 `HttpOnly`、`SameSite=Strict` 的 session cookie，修改操作还需要 CSRF token。
...
Web 页面通过 SSE 实时接收当前状态和结构化日志，支持按级别筛选。断线后浏览器会自动重连并补发进程内保留的日志。

日志同时写入：

- stdout/stderr，供终端或 systemd journal 使用。
- `.data/logs/YYYY-MM-DD.jsonl`，保留最近 7 个日志文件，供服务重启后在 Web 页面查看。
...
`pnpm run smoke:web` 使用本地临时服务验证 challenge 登录、session、CSRF、配置持久化、SSE 和 Worker 控制，不会连接真实游戏账户。
