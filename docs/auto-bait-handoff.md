...
当前目录已经是独立 Git 仓库。不要回到旧的 `tampermonkey-scripts` 目录继续修改。

## 2026-07-15 无头服务修正

页面注入操作面板的方案已经移除。项目目标是服务器无头长期运行，因此：

- `.env` 是唯一运行配置来源，修改后需要重启服务。
- 旧版 `.data/settings.json` 不再读取。
- 运行状态、当前目标、关键事件、异常和抛竿次数通过结构化控制台日志输出。
- 下文原有的面板设计已按当前实现改写，不应恢复 Shadow DOM 面板或浏览器内设置入口。
...
3. 继续遵守既有约束：
   ...
   - 通过页面控件操作，不直接调用游戏接口。
   - 尽量不依赖英文文本，兼容汉化脚本。
   - 功能按独立 feature 扩展，设置通过 `.env` 提供，主要状态输出到控制台。
...
- 将 `config` 已校验的环境变量映射为 feature 分组的只读快照。
- 已包含 `features.bait` 设置：
...
- `src/core/runtime-settings.js`：已改为仅从环境配置生成只读 feature 设置。
- `src/core/status-reporter.js`：已输出结构化服务日志并对连续相同状态去重。
...
- 当前目录已经是 Git 仓库，修改前后按正常 status、diff 和定向验证流程处理。
