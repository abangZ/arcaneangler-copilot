...
- 当前交互入口是 `.env` 配置和控制台日志，不在游戏页面注入操作面板。
...
- `.env` 是运行配置的唯一来源，`src/config.js` 负责解析和校验。
- `src/core/runtime-settings.js` 只把已校验配置映射成 feature 分组的只读快照，不恢复运行时设置持久化。
- 旧版 `.data/settings.json` 不参与运行；除非用户明确改变产品方向，不要重新引入浏览器内设置面板。
- 主要运行信息统一通过 `StatusReporter` 输出到 stdout/stderr，包含 level、phase、feature、target、message 和累计抛竿次数。
...
- `VerificationFeature`、`BaitFeature`、`FishingFeature` 按优先级依次处理，不要把 DOM Locator 放进 feature。
